...
if __name__=='__main__':
    ...
    viral_annotation_files = {
                    'deepvirfinder':os.path.join(annotation_directory,'all.DVF.predictions.txt.gz'),
                    'voghmm':os.path.join(annotation_directory,'all.hmmVOG.tbl.gz'),
                    'micompletehmm':os.path.join(annotation_directory,'all.hmmMiComplete105.tbl.gz'),
                    }
    ...
    rf_model_file = os.path.join(os.path.dirname(os.path.dirname(current_path)), 'dbs/RF_model.python39.sav')
    ...
    # additional bin fasta
    bins_all = {binname:clusters[binname] for binname in [row[1] for row in RF_results.RF_predictions]}
    write_concat_bins(os.path.join(args.directoryout,'vamb_bins_all_ge5000'), bins_all, fastadict, compressed=False, maxbins=len(bins_all), minsize=5000)
    ######################
